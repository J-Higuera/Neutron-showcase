...
export default function Orbits() {
  ...
  // The ring is the heaviest thing on the page (WebGL init + one texture per world).
  ...
  const orbits = useMemo(() => {
    let counter = 0;
    return ORBIT_ORDER.map(category => {
      ...
      // Numbered over the POPULATED orbits, not their position in ORBIT_ORDER:
      // retiring a category used to leave a gap in the printed sequence.
      .map((o, i) => ({ ...o, number: String(i + 1).padStart(2, '0') }));
  }, [entries]);
