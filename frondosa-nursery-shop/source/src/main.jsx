...
const products = [
  ...
  p('zz-plant','ZZ Plant','Zamioculcas zamiifolia','foliage',44,19,'6-inch grower pot','ships 12-18 in. tall','medium','low/artificial','no-fuss','dry rooms accepted','keep away from pets / toxic',true,['winter-ship'],img.snake,'Glossy low-light plant with rhizomes that store water.','Water sparingly and avoid standing moisture.','A reliable office bench staple.'),
  p('parlor-palm','Parlor Palm','Chamaedorea elegans','palm',48,10,'6-inch grower pot','ships 16-24 in. tall','medium','low/artificial','easy','average humidity','pet-friendly',true,['giftable'],img.fern,'Soft palm texture for bedrooms and shaded living rooms.','Even moisture with a short dry-down between waterings.','Leaflets are sleeved upright for transit.'),
  p('rubber-tree-burgundy','Burgundy Rubber Tree','Ficus elastica','foliage',62,9,'8-inch grower pot','ships 22-30 in. tall','floor/statement','medium/bright indirect','easy','average humidity','keep away from pets / toxic',true,['air-cleaner'],img.fiddle,'Deep glossy leaves with strong modern silhouette.','Water when top half dries; wipe leaves monthly.','Ships with a stake if the stem is young.'),
  p('string-of-hearts','String of Hearts','Ceropegia woodii','trailing',30,13,'4-inch hanging pot','ships 10-18 in. trailing','shelf/trailing','medium/bright indirect','easy','dry rooms accepted','pet-friendly',false,['giftable'],img.pothos,'Fine trailing vines with heart-shaped leaves for bright shelves.','Let soil mostly dry; avoid heavy pots.','Vines are looped in tissue for shipping.'),
  p('bird-nest-fern','Bird Nest Fern','Asplenium nidus','fern',32,16,'4-inch grower pot','ships 8-12 in. tall','tabletop','medium/bright indirect','moderate','likes humidity','pet-friendly',true,['giftable'],img.fern,'Rippled fronds from a central rosette.','Water soil edge, not the crown.','Great bathroom plant when light is filtered.'),
  p('jade-plant','Jade Plant','Crassula ovata','succulent/cactus',24,17,'4-inch terracotta pot','ships 5-8 in. tall','tabletop','direct sun','no-fuss','dry air is fine','keep away from pets / toxic',true,['winter-ship'],img.succulent,'Sturdy succulent with thick oval leaves.','Bright sun and infrequent water keep stems compact.','Travels well when soil is dry.'),
  p('hoya-carnosa','Hoya Carnosa','Hoya carnosa','trailing',36,7,'5-inch hanging pot','ships 10-16 in. trailing','shelf/trailing','medium/bright indirect','easy','average humidity','pet-friendly',true,['giftable'],img.pothos,'Wax plant with sturdy vines and sweet blooms when mature.','Let potting mix dry halfway; avoid overpotting.','Ships with vine loops pinned to paper.'),
  p('pilea-peperomioides','Chinese Money Plant','Pilea peperomioides','foliage',26,20,'4-inch grower pot','ships 6-10 in. tall','tabletop','medium/bright indirect','easy','average humidity','pet-friendly',true,['giftable'],img.maranta,'Round coin leaves on bouncy stems; easy to share pups.','Water when top half dries and rotate often.','Packed with a small nursery support ring.'),
  p('orchid-mini','Mini Phalaenopsis Orchid','Phalaenopsis hybrid','blooming',46,5,'3-inch orchid pot','ships 10-14 in. tall','tabletop','medium/bright indirect','moderate','likes humidity','pet-friendly',false,['giftable'],img.lily,'Compact blooming orchid for desks and gifting.','Water bark mix weekly after checking root color.','Spike is clipped and bloom-safe packed.'),
  p('aloe-vera','Aloe Vera','Aloe barbadensis miller','succulent/cactus',20,24,'4-inch grower pot','ships 6-9 in. tall','tabletop','direct sun','no-fuss','dry air is fine','keep away from pets / toxic',true,['winter-ship'],img.succulent,'Sun-loving medicinal succulent with upright pups.','Dry fully between waterings.','Ships dry to protect roots.'),
  ...
  p('pet-safe-shelf-bundle','Pet-Safe Shelf Bundle','Maranta, Spider Plant, Hoya','bundle',76,10,'three 4-inch pots','ships 3 plants','shelf/trailing','medium/bright indirect','easy','average to humid','pet-friendly',true,['giftable'],img.spider,'ASPCA-friendly shelf mix for homes with curious pets.','Water rhythms are close enough for one weekly check.','Each plant has a clipped tag and packing slip line.'),
  ...
  p('ceramic-cachepot-mist','Mist Ceramic Cachepot','Glazed ceramic pot','accessory',28,22,'6-inch cachepot','ships boxed','tabletop','medium/bright indirect','easy','not applicable','pet-friendly',true,['giftable'],img.calathea,'Pale mist cachepot sized for a 6-inch nursery liner.','No drainage hole; lift grower pot to water.','Foam-corner packed separate from plants.'),
  p('terracotta-saucer-kit','Terracotta Saucer Kit','Porous terracotta set','accessory',18,33,'4-inch pot + saucer','ships boxed','tabletop','direct sun','no-fuss','not applicable','pet-friendly',true,['winter-ship'],img.succulent,'Breathable pot and saucer for succulents and starts.','Best with gritty mix and bright direct light.','Wrapped to prevent chipping.'),
...
const previewPrefix = () => window.location.pathname.startsWith('/preview/frondosa-nursery-shop') ? '/preview/frondosa-nursery-shop' : '';
function path() {
  const raw = window.location.pathname.replace(/^\/preview\/frondosa-nursery-shop/, '') || '/';
  return raw !== '/' ? raw.replace(/\/+$/, '') : raw;
}
function route(to) { history.pushState(null, '', `${previewPrefix()}${to}`); window.dispatchEvent(new Event('popstate')); window.scrollTo({ top: 0, behavior: 'smooth' }); }
...
function FilterRail({ filters, setFilters, drawer, setDrawer }) {
  ...
  const panel = <aside className="filter-rail">
    <div className="rail-title"><Leaf size={19} /><div><strong>Care match tags</strong><small>Clip filters together</small></div><button className="mobile-close" onClick={() => setDrawer(false)}><X size={18} /></button></div>
    ...
    <fieldset className="tag-stack"><legend>Price up to {money(filters.price)}</legend><input className="range" type="range" min="10" max="140" value={filters.price} onChange={(e) => setFilters({ ...filters, price: Number(e.target.value) })} /></fieldset>
...
function ProductCard({ product, addToCart }) {
  return <article className="product-card">
    <button className="photo-button" onClick={() => route(`/plant/${product.id}`)} aria-label={`View care card for ${product.name}`}><img src={product.image} alt={`${product.name} product photo`} /></button>
  ...
  return <section className="detail-shell full-product">
    ...
    <div className="product-hero">
      <div className="gallery showcase-gallery">
        ...
        <div>{product.gallery.map((src, i) => <button className={active === i ? 'active' : ''} key={src} onClick={() => setActive(i)}><img src={src} alt="" /></button>)}</div>
      </div>
...
function Checkout({ cartLines, totals, clearCart, orders, setOrders, setLoggedIn }) {
  ...
  const submit = (e) => {
    ...
    setErrors(next);
    if (Object.keys(next).length) { document.querySelector('.error-summary')?.focus(); return; }
  ...
  return <section className="checkout-page">
    ...
    <form className="checkout-form" onSubmit={submit} noValidate>
      ...
      {['name','email','address','city','state','zip'].map((k) => <label key={k}>{k === 'zip' ? 'ZIP / postal' : k[0].toUpperCase() + k.slice(1)}<input value={form[k]} onChange={(e) => setForm({ ...form, [k]: e.target.value })} aria-invalid={!!errors[k]} autoComplete={k === 'name' ? 'name' : k === 'email' ? 'email' : k === 'address' ? 'street-address' : 'shipping ' + k} />{errors[k] && <small className="field-error">{errors[k]}</small>}</label>)}
