...
(() => {
  ...
  /* ---------- anchor nav: programmatic smooth scroll ----------
     Native fragment navigation drops a click that lands while a previous
     smooth glide is still running (measured: second nav click swallowed,
     URL and position end up disagreeing). scrollIntoView() supersedes an
     in-flight glide reliably; behavior:'auto' follows the CSS
     scroll-behavior, so reduced-motion still gets an instant jump.
     The skip link keeps native navigation for its focus semantics. */
  ...
  if (routesG) {
    ...
    if (!reduce) {
      ...
      // run only while the lattice is on screen and the tab is visible
      let latticeInView = true;
      const syncPulses = () => {
        const shouldRun = latticeInView && !document.hidden;
      ...
      const pio = new IntersectionObserver((es) => {
        es.forEach((e) => { latticeInView = e.isIntersecting; });
        syncPulses();
      });
      pio.observe($('.lattice__frame'));
      document.addEventListener('visibilitychange', syncPulses);
      syncPulses();
    }
  }
